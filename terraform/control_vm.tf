data "template_file" "control_vm_cloud-init" {
  ...
  vars = {
    hostname            = "${var.rke_cluster_name}-rke-controlVm-${count.index}"
...
resource "nutanix_virtual_machine" "control_vm" {
  count                = local.amount_of_control_vms
  name                 = "${var.rke_cluster_name}-rke-controlVm-${count.index}"
  cluster_uuid         = local.cluster_uuid
  num_vcpus_per_socket = var.rke_control_vm_config.num_vcpus_per_socket
  num_sockets          = var.rke_control_vm_config.num_sockets
  memory_size_mib      = var.rke_control_vm_config.memory_size_mib
  disk_list {
    ...
    disk_size_bytes = var.rke_control_vm_config.disk_size_mib * 1024 * 1024
  }
  ...
  categories {
    ...
    value = nutanix_category_value.rke_node_type_control.id
  }
  categories {
