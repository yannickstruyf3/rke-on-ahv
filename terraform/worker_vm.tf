data "template_file" "worker_vm_cloud-init" {
  count    = var.amount_of_rke_worker_vms
  template = file("${path.module}/templates/cloud-init.tpl")
  vars = {
    hostname            = "${var.rke_cluster_name}-rke-workerVm-${count.index}"
...
resource "nutanix_virtual_machine" "worker_vm" {
  count                = var.amount_of_rke_worker_vms
  name                 = "${var.rke_cluster_name}-rke-workerVm-${count.index}"
  cluster_uuid         = local.cluster_uuid
  num_vcpus_per_socket = var.rke_worker_vm_config.num_vcpus_per_socket
  num_sockets          = var.rke_worker_vm_config.num_sockets
  memory_size_mib      = var.rke_worker_vm_config.memory_size_mib
  disk_list {
    ...
    disk_size_bytes = var.rke_worker_vm_config.disk_size_mib * 1024 * 1024
  }
  ...
  categories {
    ...
    value = nutanix_category_value.rke_node_type_worker.id
  }
  categories {
    name  = nutanix_category_key.rke_admin_vm_ip.id
    value = nutanix_category_value.rke_admin_vm_ip.id
  }
  connection {
